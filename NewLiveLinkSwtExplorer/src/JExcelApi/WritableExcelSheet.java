...
public class WritableExcelSheet {
	...
	private WritableFont arial8ItalicBlueFont = new WritableFont(WritableFont.ARIAL, 8, WritableFont.NO_BOLD, true,
			UnderlineStyle.SINGLE, Colour.BLUE);

	private Display display = null;
	private LiveLinkNode rootNode = null;

	public WritableExcelSheet(final jxl.write.WritableSheet writableSheet, final Display display) {
	...
	private int writeLiveLinkNodeData(LiveLinkNode llNode, String parentNode, Boolean indentedEXCELresults,
			int maxDepth) {
					...
					if (llNode.getChildren() != null) {
						...
						while (iter.hasNext()) {
							...
							if (RowIndex < MaxNumberOfRows) {
								...
								throw new Excel2003MaxRowsException(
										"EXCEL 2003 - row index exceeds max number of row > " + (MaxNumberOfRows));
							}
	...
	public boolean writeReadMeSheet(LiveLinkNode llNode) {
		...
		writeStringValue(ColumnIndex++, RowIndex, llNode.getDescription(), false);
		writeStringValue(ColumnIndex++, RowIndex, llNode.getMimeType(), false);
		// write the size in Bytes if it is a document or the number of files if it is a
		// folder
		writeLongValue(ColumnIndex++, RowIndex, llNode.getLongSize(), false);
	...
	private boolean writeThalesImage() {
		...
		try {
			...
			if (in != null) {
				...
				// System.out.println("WritableExcelFile: Write Thales Image: width:
				// "+input.getWidth()+" height: "+input.getHeight());
	...
	/**
	 * traverse the tree and select a node tree such as the node is a folder or
	 * 
	 * @param items
	 */
	...
	private void recursiveFileExplorer(File initialFile, final StatusBarObserver statusBarObserver)
			throws Excel2003MaxRowsException {
	...
	public void writeFileExplorerRecursive(final File selectedFile, final StatusBarObserver statusBarObserver)
			throws Excel2003MaxRowsException {
	...
	public boolean writeFileExplorer(File initialFile, final StatusBarObserver statusBarObserver)
			throws Excel2003MaxRowsException {
	...
	public boolean writeFileExplorer(final File initialFile, final StatusBarObserver statusBarObserver,
			final ArrayList<File> browsedFiles)  throws Excel2003MaxRowsException{

		int ColumnIndex = writeFileExplorerHeader();
		Iterator<File> iter = browsedFiles.iterator();
		while (iter.hasNext()) {
			File file = iter.next();
			// use file observable
			FileObservable fileObservable = new FileObservable(file);
	...
	private void writeGeneratedIndex() throws Excel2003MaxRowsException {
		...
		if (liveLinkNodeTreeSet.size() > 64000) {
			new ShellErrorMessage(this.display, this.display.getActiveShell(),
					"Number of records exceed max EXCEL size !!!");
		}
		...
		while (iter.hasNext() && (this.RowIndex < 64000)) {
			...
			// this part of the excel reports is not indented :it is always aligned on the
	...
	private void buildLiveLinkNodeTreeCollection(TreeItem[] items) throws Excel2003MaxRowsException {
		...
		for (int i = 0; i < items.length; i++) {
			...
			if (item.getData() instanceof LiveLinkNode) {
				...
				LiveLinkNodeExcelRecord llXlsRecord = new LiveLinkNodeExcelRecord(this.RowIndex, treeDepth,
						strParentText, llNode);
	...
	public void generateLiveLinkNodeRecursiveIndex(LiveLinkNode llNode, boolean writeHeader,
			boolean indentedEXCELresults, int maxDepth) throws Excel2003MaxRowsException {
